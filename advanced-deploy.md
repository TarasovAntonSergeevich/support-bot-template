...
## Продвинутая инструкция по развертыванию support-bot-template
...
```sql
CREATE USER support_bot_template_user PASSWORD '<GENERATE>';
CREATE DATABASE support_bot_template_db;
GRANT ALL PRIVILEGES ON DATABASE support_bot_template_db 
  TO support_bot_template_user;
```

2. Скачайте репозиторий на сервер:

```bash
git clone <THIS_REPOSITORY> /opt/express/bots/support-bot-template
cd /opt/express/bots/support-bot-template
```

3. Соберите образ:

```bash
docker build -t support-bot-template .
```
...
4. В директории `/opt/express/bots/support-bot-template` создайте файл
